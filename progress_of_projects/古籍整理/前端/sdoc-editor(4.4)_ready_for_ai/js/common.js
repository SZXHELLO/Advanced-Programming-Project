...
const XMLGenerator = {
    generate() {
        ...
        AppState.pages.forEach((page, i) => {
            ...
            const rootChars = pageAnnotations.filter(a => a.attrType === '字' && !a.parentId);
            const rootWords = pageAnnotations.filter(a => a.attrType === '词' && !a.parentId);
            const sentences = pageAnnotations.filter(a => a.attrType === '句子');

            xml += `    <page id="${page.id}" page_no="${i + 1}">\n`;
            xml += `      <panel>\n`;
            rootChars.forEach(ann => {
                xml += this.renderTextfield(ann, 8);
            });
            ...
            sentences.forEach(sentence => {
                const children = (sentence.childIds || [])
                    ...
                    .filter(Boolean);
                xml += this.renderTextfield(sentence, 8, children);
            });
            xml += `      </panel>\n`;
            xml += `    </page>\n`;
        });
