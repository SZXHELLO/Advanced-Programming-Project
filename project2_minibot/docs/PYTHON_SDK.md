...
Use minibot programmatically — load config, run the agent, get results.
...
### `Minibot.from_config(config_path?, *, workspace?)`

Create a `Minibot` from a config file.

| Param | Type | Default | Description |
|-------|------|---------|-------------|
| `config_path` | `str \| Path \| None` | `None` | Path to `config.json`. Defaults to `~/.minibot/config.json`. |
...
async def main():
    bot = Minibot.from_config(workspace="/my/project")
