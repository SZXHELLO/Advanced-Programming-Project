...
from minibot.providers.openai_responses import (
    consume_sdk_stream,
    convert_messages,
    convert_tools,
...
if TYPE_CHECKING:
    from minibot.providers.registry import ProviderSpec
...
_DEFAULT_OPENROUTER_HEADERS = {
    "HTTP-Referer": "https://github.com/HKUDS/minibot",
...
def _uses_openrouter_attribution(spec: "ProviderSpec | None", api_base: str | None) -> bool:
    """Apply Minibot attribution headers to OpenRouter requests by default."""
...
class OpenAICompatProvider(LLMProvider):
    ...
    @classmethod
    def _is_reasoning_roundtrip_error_text(cls, text: str | None) -> bool:
        """True when provider text indicates missing echoed ``reasoning_content``."""
        if not text:
            return False
        low = text.lower()
        return "reasoning_content" in low and "passed back" in low

    @classmethod
    def _is_reasoning_roundtrip_error(cls, exc: Exception) -> bool:
        ...
        """
        body = getattr(exc, "body", None) or getattr(exc, "doc", None)
        combo = " ".join(
            str(part) for part in (body, exc) if part is not None
        ).lower()
        return "reasoning_content" in combo and "passed back" in combo

    @staticmethod
    def _repair_messages_for_reasoning_roundtrip(
        messages: list[dict[str, Any]],
    ) -> list[dict[str, Any]]:
        """Strip assistant/tool turns incompatible with reasoning round-trip APIs.

        Drop any assistant row without non-empty ``reasoning_content``.
        Tool-calling assistants are included: thinking gateways still require the
        prior reasoning blob; histories saved before we persisted it cannot be replayed.

        Omit following ``tool`` messages whose ``tool_call_id`` belonged to dropped
        assistant ``tool_calls`` so the transcript stays valid.

        Keeps assistants that already carry substantive ``reasoning_content``.
        """
        def _ids_from_tool_calls(tc_list: Any) -> set[str]:
            out: set[str] = set()
        ...
        def _valid_reasoning(msg: dict[str, Any]) -> bool:
            rc = msg.get("reasoning_content")
            return isinstance(rc, str) and bool(rc.strip())
        ...
        while i < n:
            ...
            if role != "assistant":
                ...
                i += 1
                continue
            if _valid_reasoning(msg):
                repaired.append(msg)
                i += 1
                continue

            omit_ids = _ids_from_tool_calls(msg.get("tool_calls"))
            i += 1
            while omit_ids and i < n and messages[i].get("role") == "tool":
                tcid = str(messages[i].get("tool_call_id") or "")
                if tcid in omit_ids:
                    omit_ids.discard(tcid)
                    i += 1
                else:
                    break
            continue
    ...
    async def _retry_chat_after_reasoning_repair(
        self,
        messages: list[dict[str, Any]],
        *,
        repaired: list[dict[str, Any]],
        ...
        """One retry build after stripping bad history; ``None`` if nothing changed."""
        ...
        try:
            if stream:
                ...
                chunks: list[Any] = []
                stream_iter = stream_resp.__aiter__()
                while True:
                    try:
    ...
    async def chat(
        ...
        try:
            ...
            if (
                ...
                and self._is_reasoning_roundtrip_error_text(parsed.content)
            ):
                repaired = self._repair_messages_for_reasoning_roundtrip(messages)
                if repaired != messages:
                    kwargs2 = self._build_kwargs(
                        repaired, tools, model, max_tokens, temperature,
                        reasoning_effort, tool_choice,
                    )
                    return self._parse(await self._client.chat.completions.create(**kwargs2))
            return parsed
    ...
    async def chat_stream(
        ...
        idle_timeout_s = int(os.environ.get("MINIBOT_STREAM_IDLE_TIMEOUT_S", "90"))
        try:
            if self._should_use_responses_api(model, reasoning_effort):
                try:
                    body = self._build_responses_body(
            ...
            if (
                ...
                repaired = self._repair_messages_for_reasoning_roundtrip(messages)
                retry = await self._retry_chat_after_reasoning_repair(
                    messages,
                    repaired=repaired,
                    tools=tools,
                    model=model,
