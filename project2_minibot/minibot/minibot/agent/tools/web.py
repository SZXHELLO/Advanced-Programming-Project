...
from minibot.utils.helpers import build_image_content_blocks
...
class WebSearchTool(Tool):
    ...
    async def _search_duckduckgo(self, query: str, n: int) -> str:
        try:
            ...
            logger.warning("DuckDuckGo search via DDGS failed: {}; trying HTML fallback", e)
            return await self._search_duckduckgo_html_fallback(query, n, e)

    async def _search_duckduckgo_html_fallback(
        self,
        query: str,
        n: int,
        original_error: Exception,
    ) -> str:
        """Fallback search using DuckDuckGo HTML endpoint when DDGS fails."""
        try:
            async with httpx.AsyncClient(proxy=self.proxy, follow_redirects=True) as client:
                ...
                r.raise_for_status()
            items = self._parse_duckduckgo_html_results(r.text, n)
            if items:
                return _format_results(query, items, n)
            return f"No results for: {query}"
        except Exception as fallback_error:
            logger.warning(
                "DuckDuckGo HTML fallback failed (ddgs_error={}, fallback_error={})",
                original_error,
                fallback_error,
            )
            return f"Error: DuckDuckGo search failed ({original_error})"

    @staticmethod
    def _parse_duckduckgo_html_results(html_text: str, n: int) -> list[dict[str, str]]:
        """Parse top results from DuckDuckGo's HTML search page."""
        blocks = re.findall(
            r'<a[^>]*class="result__a"[^>]*href="([^"]+)"[^>]*>(.*?)</a>([\s\S]*?)(?=<a[^>]*class="result__a"|$)',
            html_text,
            flags=re.I,
        )
        items: list[dict[str, str]] = []
        for href, title_html, tail_html in blocks:
            ...
            snippet = _normalize(_strip_tags(snippet_match.group(1) if snippet_match else ""))
            if not title:
                continue
        ...
        try:
            async with httpx.AsyncClient(proxy=self.proxy, follow_redirects=True, max_redirects=MAX_REDIRECTS, timeout=15.0) as client:
                async with client.stream("GET", url, headers={"User-Agent": USER_AGENT}) as r:
                    from minibot.security.network import validate_resolved_url
