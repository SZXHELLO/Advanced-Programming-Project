...
from minibot.providers.base import LLMProvider, ToolCallRequest
from minibot.utils.helpers import (
...
from minibot.utils.runtime import (
...
class AgentRunner:
    ...
    async def _run_tool(
        ...
        async def _maybe_observe(event: dict[str, str]) -> None:
            """Emit Observation (tool result) to the progress callback."""
            cb = spec.progress_callback
            if cb is None:
                return
            content = (
                f"Observation ({event.get('status', 'ok')}): "
                f"{event.get('name', 'tool')} => {event.get('detail', '')}"
            )
            ret = cb(content, tool_hint=False)
