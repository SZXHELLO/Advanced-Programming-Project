...
- Publish to Minibot bus via BaseChannel._handle_message()
...
def _make_bot_class(channel: QQChannel) -> type[botpy.Client]:
    ...
    class _Bot(botpy.Client):
        def __init__(self):
            # Disable botpy's file log — minibot uses loguru; default "botpy.log" fails on read-only fs
...
class QQConfig(Base):
    ...
    # Optional: directory to save inbound attachments. If empty, use minibot get_media_dir("qq").
    media_dir: str = ""
    ...
    def _init_media_root(self) -> Path:
        ...
        if self.config.media_dir:
            ...
            try:
                ...
                root = Path.home() / ".minibot" / "media" / "qq"
        else:
            root = Path.home() / ".minibot" / "media" / "qq"
