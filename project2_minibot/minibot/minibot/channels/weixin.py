...
from minibot.config.paths import get_media_dir, get_runtime_subdir
from minibot.config.schema import Base
from minibot.utils.helpers import split_message
...
class WeixinConfig(Base):
    ...
    state_dir: str = ""  # Default: ~/.minibot/weixin/
    ...
    async def start(self) -> None:
        ...
        if self.config.token:
            ...
            if not await self._qr_login():
                logger.error("WeChat login failed. Run 'minibot channels login weixin' to authenticate.")
                self._running = False
                return
