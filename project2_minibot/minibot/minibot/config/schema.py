...
class QQWebhookConfig(Base):
    ...
    access_token: str = ""
    allow_from: list[str] = Field(default_factory=lambda: ["*"])
    # NapCat (and current go-cqhttp) accept ``message`` as a JSON array of segments
    # with ``type: file`` / ``image`` and a local path — real files are delivered in QQ.
    # Set False only if you need the legacy behaviour (paths appended as plain text).
    send_media_as_onebot_segments: bool = True
...
class ToolsConfig(Base):
    ...
    # Optional list of python module paths to load local tool plugins.
    # Each plugin module can register Tool classes via a decorator
    # (see minibot.agent.tools.plugin_loader.tool_plugin).
...
class Config(BaseSettings):
    ...
    def _match_provider(
        ...
        from minibot.providers.registry import PROVIDERS, find_by_name
    ...
    model_config = ConfigDict(env_prefix="MINIBOT_", env_nested_delimiter="__")
