...
from minibot.cli import commands
from minibot.cli import stream as stream_mod
...
def mock_prompt_session():
    ...
    with patch("minibot.cli.commands._PROMPT_SESSION", mock_session), \
         patch("minibot.cli.commands.patch_stdout"):
        yield mock_session
...
def test_init_prompt_session_creates_session():
    ...
    with patch("minibot.cli.commands.PromptSession") as MockSession, \
         patch("minibot.cli.commands.FileHistory") as MockHistory, \
...
def test_print_cli_progress_line_observation_without_arrow():
    """ReAct environment Observation lines print as plain text (no ↳ / dim prefix)."""
    with patch.object(commands.console, "print") as mock_print:
        commands._print_cli_progress_line("Observation: tool returned ok", None)
    mock_print.assert_called_once_with("Observation: tool returned ok")


def test_print_cli_progress_line_fact_alignment_without_arrow():
    with patch.object(commands.console, "print") as mock_print:
        commands._print_cli_progress_line("事实对齐: reasoning matches.\n", None)
    mock_print.assert_called_once_with("事实对齐: reasoning matches.\n")


def test_print_cli_progress_line_round_header_keeps_arrow():
    with patch.object(commands.console, "print") as mock_print:
        commands._print_cli_progress_line("第一轮循环\nThought: x", None)
...
async def test_print_interactive_progress_line_pauses_spinner_before_printing():
    ...
    with patch("minibot.cli.commands._print_interactive_line", side_effect=fake_print):
        thinking = stream_mod.ThinkingSpinner(console=mock_console)
        with thinking:
...
@pytest.mark.asyncio
async def test_drain_subagent_handoffs_runs_loop_until_subagents_finish():
    ...
    loop = _FakeLoop()
    await commands._drain_subagent_handoffs_once(
        loop,
        poll_interval=0.0,
        settle_delay=0.0,
        timeout=1.0,
...
@pytest.mark.asyncio
async def test_drain_subagent_handoffs_skips_when_no_running_subagents():
    class _FakeSubagents:
        def get_running_count(self) -> int:
            return 0
