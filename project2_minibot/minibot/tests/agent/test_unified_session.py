...
from minibot.command.builtin import cmd_new, register_builtin_commands
from minibot.command.router import CommandContext, CommandRouter
from minibot.config.schema import AgentDefaults, Config
...
def _make_loop(tmp_path: Path, unified_session: bool = False) -> AgentLoop:
    ...
    with patch("minibot.agent.loop.SessionManager"), \
         patch("minibot.agent.loop.SubagentManager") as MockSubMgr, \
         patch("minibot.agent.loop.Dream"):
...
class TestUnifiedSessionConfig:
    ...
    def test_onboard_generated_config_contains_unified_session(self, tmp_path: Path):
        """save_config() writes 'unifiedSession' into config.json (simulates minibot onboard)."""
        from minibot.config.loader import save_config
...
class TestConsolidationUnaffectedByUnifiedSession:
    ...
    async def test_consolidation_skips_empty_session_for_unified_key(self):
        ...
        from minibot.agent.memory import Consolidator, MemoryStore
...
class TestStopCommandWithUnifiedSession:
    ...
    async def test_stop_command_finds_task_in_unified_mode(self, tmp_path: Path):
        ...
        from minibot.agent.loop import UNIFIED_SESSION_KEY
        from minibot.command.builtin import cmd_stop
