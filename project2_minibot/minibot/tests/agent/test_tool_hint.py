"""Tests for tool hint formatting (minibot.utils.tool_hints)."""

from minibot.utils.tool_hints import format_tool_hints
from minibot.providers.base import ToolCallRequest
...
class TestToolHintKnownTools:
    ...
    def test_read_file_long_path(self):
        result = _hint([_tc("read_file", {"path": "/home/user/.local/share/uv/tools/minibot/agent/loop.py"})])
    ...
    def test_exec_abbreviates_paths_in_command(self):
        ...
        cmd = "cd D:\\Documents\\GitHub\\minibot\\.worktree\\tomain\\minibot && git diff origin/main...pr-2706 --name-only 2>&1"
        result = _hint([_tc("exec", {"command": cmd})])
    ...
    def test_exec_abbreviates_linux_paths(self):
        ...
        cmd = "cd /home/user/projects/minibot/.worktree/tomain && make build"
        result = _hint([_tc("exec", {"command": cmd})])
        assert "\u2026/" in result
    ...
    def test_exec_abbreviates_home_paths(self):
        ...
        cmd = "cd ~/projects/minibot/workspace && pytest tests/"
        result = _hint([_tc("exec", {"command": cmd})])
        assert "\u2026/" in result
