...
from minibot.providers.base import LLMResponse, ToolCallRequest
...
def _make_loop(tmp_path):
    ...
    with patch("minibot.agent.loop.ContextBuilder"), \
         patch("minibot.agent.loop.SessionManager"), \
         patch("minibot.agent.loop.SubagentManager") as MockSubMgr:
...
@pytest.mark.asyncio
async def test_runner_emits_tool_observation_via_progress_callback():
    """Runner should call progress_callback with Observation after tools."""
    from minibot.agent.runner import AgentRunSpec, AgentRunner
    ...
    async def on_progress(content: str, *, tool_hint: bool = False) -> None:
        if not tool_hint and content.startswith("Observation"):
    ...
    assert result.final_content == "done"
    assert observations == ["Observation (ok): list_dir => tool result"]
...
async def test_runner_persists_large_tool_results_for_follow_up_calls(tmp_path):
    ...
    assert (tmp_path / ".minibot" / "tool-results" / "test_runner" / "call_big.txt").exists()


def test_persist_tool_result_prunes_old_session_buckets(tmp_path):
    from minibot.utils.helpers import maybe_persist_tool_result

    root = tmp_path / ".minibot" / "tool-results"
...
def test_persist_tool_result_logs_cleanup_failures(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "minibot.utils.helpers._cleanup_tool_result_buckets",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(OSError("busy")),
    )
    monkeypatch.setattr(
        "minibot.utils.helpers.logger.warning",
...
def test_snip_history_drops_orphaned_tool_results_from_trimmed_slice(monkeypatch):
    ...
    monkeypatch.setattr("minibot.agent.runner.estimate_prompt_tokens_chain", lambda *_args, **_kwargs: (500, None))
    token_sizes = {
        "old user": 120,
        "tool call": 120,
        "tool output": 40,
    ...
    monkeypatch.setattr(
        "minibot.agent.runner.estimate_message_tokens",
...
async def test_runner_keeps_going_when_tool_result_persistence_fails():
    ...
    runner = AgentRunner(provider)
    with patch("minibot.agent.runner.maybe_persist_tool_result", side_effect=RuntimeError("disk full")):
        result = await runner.run(AgentRunSpec(
            initial_messages=[{"role": "user", "content": "do task"}],
            tools=tools,
            model="test-model",
...
async def test_llm_error_not_appended_to_session_messages():
    ...
    from minibot.agent.runner import (
        AgentRunSpec,
        AgentRunner,
...
async def test_next_turn_after_llm_error_keeps_turn_boundary(tmp_path):
    from minibot.agent.loop import AgentLoop
    from minibot.agent.runner import _PERSISTED_MODEL_ERROR_PLACEHOLDER
    from minibot.bus.events import InboundMessage
    from minibot.bus.queue import MessageBus
...
async def test_subagent_max_iterations_announces_existing_fallback(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("minibot.agent.tools.filesystem.ListDirTool.execute", fake_execute)

    await mgr._run_subagent(
        "sub-1", "do task", "label", {"channel": "test", "chat_id": "c1"}, "test:c1",
    )
...
async def test_length_recovery_gives_up_after_max_retries():
    ...
    from minibot.agent.runner import AgentRunSpec, AgentRunner, _MAX_LENGTH_RECOVERIES
...
async def test_backfill_missing_tool_results_inserts_error():
    ...
    from minibot.agent.runner import AgentRunner, _BACKFILL_CONTENT
...
async def test_backfill_repairs_model_context_without_shifting_save_turn_boundary(tmp_path):
    ...
    from minibot.agent.loop import AgentLoop
    from minibot.agent.runner import _BACKFILL_CONTENT
    from minibot.bus.events import InboundMessage
    from minibot.bus.queue import MessageBus
...
async def test_runner_backfill_only_mutates_model_context_not_returned_messages():
    ...
    from minibot.agent.runner import AgentRunSpec, AgentRunner, _BACKFILL_CONTENT
...
async def test_microcompact_replaces_old_tool_results():
    ...
    from minibot.agent.runner import AgentRunner, _MICROCOMPACT_KEEP_RECENT
...
async def test_drain_injections_passes_limit_to_callback_when_supported():
    ...
    from minibot.agent.runner import AgentRunSpec, AgentRunner, _MAX_INJECTIONS_PER_TURN
    from minibot.bus.events import InboundMessage
...
async def test_injection_cycles_capped_at_max():
    ...
    from minibot.agent.runner import AgentRunSpec, AgentRunner, _MAX_INJECTION_CYCLES
    from minibot.bus.events import InboundMessage
...
async def test_followup_routed_to_pending_queue(tmp_path):
    ...
    from minibot.agent.loop import UNIFIED_SESSION_KEY
    from minibot.bus.events import InboundMessage
...
async def test_pending_queue_preserves_overflow_for_next_injection_cycle(tmp_path):
    ...
    from minibot.bus.events import InboundMessage
    from minibot.bus.queue import MessageBus
    from minibot.agent.runner import _MAX_INJECTIONS_PER_TURN
...
async def test_drain_injections_on_empty_final_response():
    ...
    from minibot.agent.runner import AgentRunSpec, AgentRunner, _MAX_EMPTY_RETRIES
    from minibot.bus.events import InboundMessage
...
async def test_drain_injections_set_flag_when_followup_arrives_after_last_iteration():
    ...
    from minibot.agent.hook import AgentHook
    from minibot.agent.runner import AgentRunSpec, AgentRunner
    from minibot.bus.events import InboundMessage
