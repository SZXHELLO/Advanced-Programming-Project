...
from minibot.channels.discord import (
    MAX_MESSAGE_LEN,
    DiscordBotClient,
    DiscordChannel,
    DiscordConfig,
)
from minibot.command.builtin import build_help_text
...
async def test_start_returns_when_discord_dependency_missing(monkeypatch) -> None:
    ...
    monkeypatch.setattr("minibot.channels.discord.DISCORD_AVAILABLE", False)
...
async def test_start_handles_client_construction_failure(monkeypatch) -> None:
    ...
    monkeypatch.setattr("minibot.channels.discord.DiscordBotClient", _boom)
...
async def test_start_handles_client_start_failure(monkeypatch) -> None:
    ...
    monkeypatch.setattr("minibot.channels.discord.DiscordBotClient", _FakeDiscordClient)
...
async def test_on_message_downloads_attachments(tmp_path, monkeypatch) -> None:
    ...
    channel._handle_message = capture_handle  # type: ignore[method-assign]
    monkeypatch.setattr("minibot.channels.discord.get_media_dir", lambda _name: tmp_path)
...
async def test_send_delta_streams_by_editing_message(monkeypatch) -> None:
    ...
    monkeypatch.setattr("minibot.channels.discord.time.monotonic", lambda: next(times, 5.0))
...
async def test_send_delta_stream_end_splits_oversized_reply(monkeypatch) -> None:
    ...
    times = iter([1.0, 3.0])
    monkeypatch.setattr("minibot.channels.discord.time.monotonic", lambda: next(times, 3.0))
