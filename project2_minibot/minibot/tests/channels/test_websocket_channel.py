...
from minibot.channels.websocket import (
    WebSocketChannel,
    WebSocketConfig,
...
async def _http_get(url: str, headers: dict[str, str] | None = None) -> httpx.Response:
    ...
    return await asyncio.to_thread(
        functools.partial(
            httpx.get, url, headers=headers or {}, timeout=5.0, trust_env=False
        )
    )
...
def test_issue_route_secret_matches_bearer_and_header() -> None:
    ...
    x_headers = Headers([("X-Minibot-Auth", "my-secret")])
...
def test_registry_discovers_websocket_channel() -> None:
    from minibot.channels.registry import load_channel_class
