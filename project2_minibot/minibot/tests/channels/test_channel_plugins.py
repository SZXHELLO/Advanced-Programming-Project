...
from minibot.config.schema import ChannelsConfig
from minibot.utils.restart import RestartNotice
...
def test_discover_plugins_loads_entry_points():
    from minibot.channels.registry import discover_plugins
...
async def test_manager_loads_plugin_from_dict_config():
    ...
    with patch(
        "minibot.channels.registry.discover_all",
...
def test_channels_login_uses_discovered_plugin_class(monkeypatch):
    from minibot.cli.commands import app
    ...
    monkeypatch.setattr("minibot.config.loader.load_config", lambda config_path=None: Config())
    monkeypatch.setattr(
        "minibot.channels.registry.discover_all",
...
async def test_send_with_retry_retries_on_failure():
    ...
    with patch("minibot.channels.manager.asyncio.sleep", new_callable=AsyncMock) as mock_sleep:
        await mgr._send_with_retry(mgr.channels["failing"], msg)
...
async def test_send_with_retry_no_retry_when_max_is_zero():
    ...
    with patch("minibot.channels.manager.asyncio.sleep", new_callable=AsyncMock):
        await mgr._send_with_retry(mgr.channels["failing"], msg)
...
async def test_send_with_retry_propagates_cancelled_error_during_sleep():
    ...
    with patch("minibot.channels.manager.asyncio.sleep", side_effect=cancel_during_sleep):
        with pytest.raises(asyncio.CancelledError):
            await mgr._send_with_retry(mgr.channels["failing"], msg)
...
async def test_notify_restart_done_enqueues_outbound_message():
    ...
    with patch("minibot.channels.manager.consume_restart_notice_from_env", return_value=notice):
