"""WebSocket server channel: minibot acts as a WebSocket server and serves connected clients."""
...
class WebSocketConfig(Base):
    ...
    - ``token_issue_path``: If non-empty, **GET** (HTTP/1.1) to this path returns JSON
      ...
      minibot and shares the asyncio loop, use a thread or async HTTP client for GET—do not call
...
def _issue_route_secret_matches(headers: Any, configured_secret: str) -> bool:
    ...
    header_token = headers.get("X-Minibot-Auth") or headers.get("x-minibot-auth")
    if not header_token:
        return False
