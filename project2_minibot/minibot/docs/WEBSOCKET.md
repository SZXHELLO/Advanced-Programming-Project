...
Minibot can act as a WebSocket server, allowing external clients (web apps, CLIs, scripts) to interact with the agent in real time via persistent connections.
...
async def main():
    async with websockets.connect("ws://127.0.0.1:8765/?client_id=alice") as ws:
        ...
        await ws.send(json.dumps({"content": "Hello minibot!"}))
...
1. Client sends `GET {tokenIssuePath}` with `Authorization: Bearer {tokenIssueSecret}` (or `X-Minibot-Auth` header).
...
- An HTTP file server serving the minibot media directory
