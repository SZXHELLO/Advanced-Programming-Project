...
Build a custom minibot channel in three steps: subclass, package, install.

> **Note:** We recommend developing channel plugins against a source checkout of minibot (`pip install -e .`) rather than a PyPI release, so you always have access to the latest base-channel features and APIs.

## How It Works

minibot discovers channel plugins via Python [entry points](https://packaging.python.org/en/latest/specifications/entry-points/). When `minibot gateway` starts, it scans:

1. Built-in channels in `minibot/channels/`
2. External packages registered under the `minibot.channels` entry point group
...
```
minibot-channel-webhook/
├── minibot_channel_webhook/
...
```python
# minibot_channel_webhook/__init__.py
from minibot_channel_webhook.channel import WebhookChannel

__all__ = ["WebhookChannel"]
```

```python
# minibot_channel_webhook/channel.py
import asyncio
...
# pyproject.toml
[project]
name = "minibot-channel-webhook"
version = "0.1.0"
dependencies = ["minibot", "aiohttp"]

[project.entry-points."minibot.channels"]
webhook = "minibot_channel_webhook:WebhookChannel"
...
```bash
pip install -e .
minibot plugins list      # verify "Webhook" shows as "plugin"
minibot onboard           # auto-adds default config for detected plugins
```
...
```bash
minibot channels login <channel_name>
minibot channels login <channel_name> --force  # re-authenticate
```
...
| `default_config()` (classmethod) | Returns default config dict for `minibot onboard`. Override to declare your fields. |
...
Built-in channels use Pydantic config models (subclassing `Base` from `minibot.config.schema`). Plugin channels **must do the same**.

### Pattern

1. Define a Pydantic model inheriting from `minibot.config.schema.Base`:
...
Override `default_config()` so `minibot onboard` auto-populates `config.json`:
...
| What | Format | Example |
|------|--------|---------|
| PyPI package | `minibot-channel-{name}` | `minibot-channel-webhook` |
| Entry point key | `{name}` | `webhook` |
| Config section | `channels.{name}` | `channels.webhook` |
| Python package | `minibot_channel_{name}` | `minibot_channel_webhook` |

## Local Development

```bash
git clone https://github.com/you/minibot-channel-webhook
cd minibot-channel-webhook
pip install -e .
minibot plugins list    # should show "Webhook" as "plugin"
minibot gateway         # test end-to-end
```

## Verify

```bash
$ minibot plugins list
