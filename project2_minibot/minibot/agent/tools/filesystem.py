...
from minibot.agent.tools.schema import BooleanSchema, IntegerSchema, StringSchema, tool_parameters_schema
from minibot.agent.tools import file_state
from minibot.utils.helpers import build_image_content_blocks, detect_image_mime
    ...
    tool_parameters_schema(
        ...
        recursive=BooleanSchema(
            description=(
                ...
                "Has no effect on regular files. Default false."
            ),
        ),
        missing_ok=BooleanSchema(
            description=(
                "If true, deleting a non-existent path returns success instead of error. "
                "Useful for idempotent cleanup. Default false."
            ),
        ),
        required=["path"],
    )
...
class DeleteFileTool(_FsTool):
    """Delete a file or directory using native filesystem calls.

    Bypasses the shell entirely (no cmd.exe / bash), so Unicode filenames —
    including CJK characters that would otherwise be mangled by cmd.exe's
    ANSI code page — are handled correctly on Windows.
    """
    ...
    def description(self) -> str:
        return (
            ...
            "Prefer this over shell `rm`/`del` — it handles Unicode paths safely "
            "and respects the workspace sandbox. "
            "Pass recursive=true to delete a non-empty directory. "
            "Pass missing_ok=true for idempotent cleanup (no error if the path is gone)."
        )
    ...
    async def execute(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            # Refuse to delete the workspace root / allowed_dir root itself —
            # that would wipe the agent's entire sandbox.
            protected_roots: list[Path] = []
            if self._workspace is not None:
                try:
                    protected_roots.append(self._workspace.resolve())
                except OSError:
                    pass
            if self._allowed_dir is not None:
                try:
                    protected_roots.append(self._allowed_dir.resolve())
                except OSError:
                    pass
            if any(fp == root for root in protected_roots):
                return (
                    f"Error: Refusing to delete sandbox root {fp}. "
                    "Delete individual files or subdirectories instead."
                )

            if not fp.exists() and not fp.is_symlink():
                if missing_ok:
                    return f"Already absent: {fp}"
                return f"Error: Path not found: {path}"

            if fp.is_symlink() or fp.is_file():
                fp.unlink()
                file_state._state.pop(str(fp), None)
                return f"Successfully deleted file {fp}"

            if fp.is_dir():
                try:
                    is_empty = not any(fp.iterdir())
                except OSError as e:
                    return f"Error: Cannot inspect directory {path}: {e}"

                if is_empty:
                    fp.rmdir()
                    return f"Successfully deleted empty directory {fp}"
                ...
                for sub in fp.rglob("*"):
                    if sub.is_file() or sub.is_symlink():
                        removed_count += 1
                        file_state._state.pop(str(sub.resolve()), None)

                shutil.rmtree(fp)
                return (
                    f"Successfully deleted directory {fp} "
                    f"({removed_count} file{'s' if removed_count != 1 else ''} removed)"
                )
