"""Interactive onboarding questionnaire for minibot."""
...
from minibot.cli.models import (
    format_token_count,
...
def _show_main_menu_header() -> None:
    ...
    console.print(
        Align.center(
            f"{__logo__} [bold cyan]{__display_name__}[{__version__}][/bold cyan]"
        )
    )
    console.print()
...
def _try_auto_fill_context_window(model: BaseModel, new_model_name: str) -> None:
    ...
    Note:
        This function imports AgentDefaults from minibot.config.schema to get
...
def _get_channel_info() -> dict[str, tuple[str, type[BaseModel]]]:
    ...
    for name, channel_cls in discover_all().items():
        try:
            mod = importlib.import_module(f"minibot.channels.{name}")
