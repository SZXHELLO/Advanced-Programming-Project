"""Tests for minibot.security.network — SSRF protection and internal URL detection."""
...
from minibot.security.network import configure_ssrf_whitelist, contains_internal_url, validate_url_target
...
def test_blocks_private_ipv4(ip: str, label: str):
    with patch("minibot.security.network.socket.getaddrinfo", _fake_resolve("evil.com", [ip])):
...
def test_blocks_ipv6_loopback():
    ...
    with patch("minibot.security.network.socket.getaddrinfo", _resolver):
...
def test_allows_public_ip():
    with patch("minibot.security.network.socket.getaddrinfo", _fake_resolve("example.com", ["93.184.216.34"])):
        ok, err = validate_url_target("http://example.com/page")
        assert ok, f"Should allow public IP, got: {err}"


def test_allows_normal_https():
    with patch("minibot.security.network.socket.getaddrinfo", _fake_resolve("github.com", ["140.82.121.3"])):
        ok, err = validate_url_target("https://github.com/HKUDS/minibot")
        assert ok
...
def test_detects_curl_metadata():
    with patch("minibot.security.network.socket.getaddrinfo", _fake_resolve("169.254.169.254", ["169.254.169.254"])):
        assert contains_internal_url('curl -s http://169.254.169.254/computeMetadata/v1/')


def test_detects_wget_localhost():
    with patch("minibot.security.network.socket.getaddrinfo", _fake_resolve("localhost", ["127.0.0.1"])):
...
def test_blocks_cgnat_by_default():
    ...
    with patch("minibot.security.network.socket.getaddrinfo", _fake_resolve("ts.local", ["100.100.1.1"])):
...
def test_whitelist_does_not_affect_other_blocked():
    ...
    configure_ssrf_whitelist(["100.64.0.0/10"])
    try:
        with patch("minibot.security.network.socket.getaddrinfo", _fake_resolve("evil.com", ["10.0.0.1"])):
