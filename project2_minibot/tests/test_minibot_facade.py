"""Tests for the Minibot programmatic facade."""
...
def test_from_config_missing_file():
    with pytest.raises(FileNotFoundError):
        Minibot.from_config("/nonexistent/config.json")


def test_from_config_creates_instance(tmp_path):
    config_path = _write_config(tmp_path)
    bot = Minibot.from_config(config_path, workspace=tmp_path)
...
def test_from_config_default_path():
    ...
    with patch("minibot.config.loader.load_config") as mock_load, \
         patch("minibot.minibot._make_provider") as mock_prov:
...
def test_workspace_override(tmp_path):
    ...
    bot = Minibot.from_config(config_path, workspace=custom_ws)
    assert bot._loop.workspace == custom_ws


def test_sdk_make_provider_uses_github_copilot_backend():
    from minibot.config.schema import Config
    from minibot.minibot import _make_provider
...
def test_import_from_top_level():
    from minibot import Minibot as N, RunResult as R
    assert N is Minibot
