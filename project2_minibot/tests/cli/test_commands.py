...
from minibot.cli.commands import _make_provider, app
from minibot.config.schema import Config
from minibot.cron.types import CronJob, CronPayload
from minibot.providers.openai_codex_provider import _strip_model_prefix
...
def mock_paths():
    ...
    with patch("minibot.config.loader.get_config_path") as mock_cp, \
         patch("minibot.config.loader.save_config") as mock_sc, \
         patch("minibot.config.loader.load_config") as mock_lc, \
         patch("minibot.cli.commands.get_workspace_path") as mock_ws:
...
def test_onboard_fresh_install(mock_paths):
    ...
    assert "minibot is ready" in result.stdout
...
def test_onboard_interactive_discard_does_not_save_or_create_workspace(mock_paths, monkeypatch):
    ...
    from minibot.cli.onboard import OnboardResult

    monkeypatch.setattr(
        "minibot.cli.onboard.run_onboard",
...
def test_onboard_uses_explicit_config_and_workspace_paths(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("minibot.channels.registry.discover_all", lambda: {})
...
def test_onboard_wizard_preserves_explicit_config_in_next_steps(tmp_path, monkeypatch):
    ...
    compact_output = stripped_output.replace("\n", "")
    resolved_config = str(config_path.resolve())
    assert f'minibot agent -m "Hello!" --config {resolved_config}' in compact_output
    assert f"minibot gateway --config {resolved_config}" in compact_output
...
def test_make_provider_uses_github_copilot_backend():
    from minibot.cli.commands import _make_provider
    from minibot.config.schema import Config
...
async def test_github_copilot_provider_refreshes_client_api_key_before_chat():
    ...
    with patch("minibot.providers.openai_compat_provider.AsyncOpenAI", return_value=mock_client):
        provider = GitHubCopilotProvider(default_model="github-copilot/gpt-5.1")
...
def test_make_provider_passes_extra_headers_to_custom_provider():
    ...
    with patch("minibot.providers.openai_compat_provider.AsyncOpenAI") as mock_async_openai:
...
def mock_agent_runtime(tmp_path):
    ...
    with patch("minibot.config.loader.load_config", return_value=config) as mock_load_config, \
         patch("minibot.config.loader.resolve_config_env_vars", side_effect=lambda c: c), \
         patch("minibot.cli.commands.sync_workspace_templates") as mock_sync_templates, \
         patch("minibot.cli.commands._make_provider", return_value=object()), \
         patch("minibot.cli.commands._print_agent_response") as mock_print_response, \
         patch("minibot.bus.queue.MessageBus"), \
         patch("minibot.cron.service.CronService"), \
         patch("minibot.agent.loop.AgentLoop") as mock_agent_loop_cls:
        ...
        agent_loop.subagents = MagicMock()
        agent_loop.subagents.get_running_count.return_value = 0
...
def test_agent_config_sets_active_path(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        "minibot.config.loader.set_config_path",
        lambda path: seen.__setitem__("config_path", path),
    )
    monkeypatch.setattr("minibot.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr("minibot.cli.commands.sync_workspace_templates", lambda _path: None)
    monkeypatch.setattr("minibot.cli.commands._make_provider", lambda _config: object())
    monkeypatch.setattr("minibot.bus.queue.MessageBus", lambda: object())
    monkeypatch.setattr("minibot.cron.service.CronService", lambda _store: object())
    ...
    monkeypatch.setattr("minibot.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("minibot.cli.commands._print_agent_response", lambda *_args, **_kwargs: None)
...
def test_agent_uses_workspace_directory_for_cron_store(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("minibot.config.loader.set_config_path", lambda _path: None)
    monkeypatch.setattr("minibot.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr("minibot.cli.commands.sync_workspace_templates", lambda _path: None)
    monkeypatch.setattr("minibot.cli.commands._make_provider", lambda _config: object())
    monkeypatch.setattr("minibot.bus.queue.MessageBus", lambda: object())
    ...
    monkeypatch.setattr("minibot.cron.service.CronService", _FakeCron)
    monkeypatch.setattr("minibot.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("minibot.cli.commands._print_agent_response", lambda *_args, **_kwargs: None)
...
def test_agent_workspace_override_does_not_migrate_legacy_cron(
    ...
    monkeypatch.setattr("minibot.cli.commands._make_provider", lambda _config: object())
    monkeypatch.setattr("minibot.bus.queue.MessageBus", lambda: object())
    monkeypatch.setattr("minibot.config.paths.get_cron_dir", lambda: legacy_dir)
...
def test_agent_custom_config_workspace_does_not_migrate_legacy_cron(
    ...
    monkeypatch.setattr("minibot.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr(
        "minibot.cli.commands._print_agent_response", lambda *_args, **_kwargs: None
    )
...
def _patch_cli_command_runtime(
    ...
    *,
    ...
    monkeypatch.setattr("minibot.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr("minibot.config.loader.resolve_config_env_vars", lambda c: c)
    monkeypatch.setattr(
        "minibot.cli.commands.sync_workspace_templates",
        sync_templates or (lambda _path: None),
    )
    monkeypatch.setattr(
        "minibot.cli.commands._make_provider",
        make_provider or (lambda _config: object()),
    )

    if message_bus is not None:
        monkeypatch.setattr("minibot.bus.queue.MessageBus", message_bus)
    if session_manager is not None:
        monkeypatch.setattr("minibot.session.manager.SessionManager", session_manager)
    if cron_service is not None:
        monkeypatch.setattr("minibot.cron.service.CronService", cron_service)
    if get_cron_dir is not None:
        monkeypatch.setattr("minibot.config.paths.get_cron_dir", get_cron_dir)
...
def _patch_serve_runtime(monkeypatch, config: Config, seen: dict[str, object]) -> None:
    ...
    monkeypatch.setattr("minibot.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("minibot.api.server.create_app", _fake_create_app)
...
def test_gateway_cron_evaluator_receives_scheduled_reminder_context(
    ...
    monkeypatch.setattr("minibot.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr("minibot.cli.commands.sync_workspace_templates", lambda _path: None)
    monkeypatch.setattr("minibot.cli.commands._make_provider", lambda _config: provider)
    monkeypatch.setattr("minibot.bus.queue.MessageBus", lambda: bus)
    monkeypatch.setattr("minibot.session.manager.SessionManager", lambda _workspace: object())
    ...
    monkeypatch.setattr("minibot.cron.service.CronService", _FakeCron)
    monkeypatch.setattr("minibot.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("minibot.channels.manager.ChannelManager", _StopAfterCronSetup)
    monkeypatch.setattr(
        "minibot.utils.evaluator.evaluate_response",
...
def test_migrate_cron_store_moves_legacy_file(tmp_path: Path) -> None:
    ...
    from minibot.cli.commands import _migrate_cron_store
    ...
    with patch("minibot.config.paths.get_cron_dir", return_value=legacy_dir):
...
def test_gateway_health_endpoint_binds_and_serves_expected_responses(
    ...
    monkeypatch.setattr("minibot.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("minibot.channels.manager.ChannelManager", _FakeChannelManager)
    monkeypatch.setattr("minibot.cron.service.CronService", _FakeCronService)
    monkeypatch.setattr("minibot.heartbeat.service.HeartbeatService", _FakeHeartbeatService)
