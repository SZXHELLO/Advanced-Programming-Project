...
@pytest.mark.asyncio
async def test_process_system_subagent_message_is_treated_as_user_input(tmp_path: Path) -> None:
    loop = _make_full_loop(tmp_path)
    loop.consolidator.maybe_consolidate_by_tokens = AsyncMock(return_value=False)  # type: ignore[method-assign]
    loop._run_agent_loop = AsyncMock(return_value=(
        "ack",
        ...
        [
            ...
            {"role": "user", "content": "subagent handoff"},
    ...
    session.add_message("assistant", "previous result")
    ...
    request_messages = loop._run_agent_loop.await_args.args[0]
