...
def test_set_and_consume_restart_notice_env_roundtrip(monkeypatch):
    monkeypatch.delenv("MINIBOT_RESTART_NOTIFY_CHANNEL", raising=False)
    monkeypatch.delenv("MINIBOT_RESTART_NOTIFY_CHAT_ID", raising=False)
    monkeypatch.delenv("MINIBOT_RESTART_STARTED_AT", raising=False)
    ...
    assert "MINIBOT_RESTART_NOTIFY_CHANNEL" not in os.environ
    assert "MINIBOT_RESTART_NOTIFY_CHAT_ID" not in os.environ
    assert "MINIBOT_RESTART_STARTED_AT" not in os.environ


def test_format_restart_completed_message_with_elapsed(monkeypatch):
    monkeypatch.setattr("minibot.utils.restart.time.time", lambda: 102.0)
