...
try:
    from minibot.channels import msteams
    MSTEAMS_AVAILABLE = getattr(msteams, "MSTEAMS_AVAILABLE", False)
except ImportError:
    MSTEAMS_AVAILABLE = False

if not MSTEAMS_AVAILABLE:
    pytest.skip("MSTeams dependencies not installed (PyJWT, cryptography). Run: pip install minibot-ai[msteams]", allow_module_level=True)

import jwt
from cryptography.hazmat.primitives.asymmetric import rsa

import minibot.channels.msteams as msteams_module
from minibot.bus.events import OutboundMessage
from minibot.channels.msteams import ConversationRef, MSTeamsChannel, MSTeamsConfig
...
def make_channel(tmp_path, monkeypatch):
    monkeypatch.setattr("minibot.channels.msteams.get_workspace_path", lambda: tmp_path)
...
def test_strip_possible_bot_mention_removes_generic_at_tags(make_channel):
    ...
    assert ch._strip_possible_bot_mention("<at>Minibot</at> hello") == "hello"
...
def test_sanitize_inbound_text_keeps_normal_inline_message(make_channel):
    ...
    activity = {
        "text": "<at>Minibot</at> normal inline message",
        "channelData": {},
    }
...
async def test_start_logs_install_hint_when_pyjwt_missing(make_channel, monkeypatch):
    ...
    assert errors == ["PyJWT not installed. Run: pip install minibot-ai[msteams]"]
