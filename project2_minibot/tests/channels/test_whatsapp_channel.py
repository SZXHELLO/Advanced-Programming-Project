...
from minibot.channels.whatsapp import (
    WhatsAppChannel,
...
def test_configured_bridge_token_skips_local_token_file(monkeypatch, tmp_path):
    token_path = tmp_path / "whatsapp-auth" / "bridge-token"
    monkeypatch.setattr("minibot.channels.whatsapp._bridge_token_path", lambda: token_path)
...
async def test_login_exports_effective_bridge_token(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("minibot.channels.whatsapp._bridge_token_path", lambda: token_path)
    monkeypatch.setattr("minibot.channels.whatsapp._ensure_bridge_setup", lambda: bridge_dir)
    monkeypatch.setattr("minibot.channels.whatsapp.shutil.which", lambda _: "/usr/bin/npm")

    def fake_run(*args, **kwargs):
        calls.append((args, kwargs))
        return MagicMock()

    monkeypatch.setattr("minibot.channels.whatsapp.subprocess.run", fake_run)
    ch = WhatsAppChannel({"enabled": True}, MagicMock())
