...
try:
    from minibot.channels import dingtalk
...
import minibot.channels.dingtalk as dingtalk_module
from minibot.channels.dingtalk import DingTalkChannel, MinibotDingTalkHandler
from minibot.channels.dingtalk import DingTalkConfig
...
async def test_group_send_uses_group_messages_api() -> None:
    ...
    ok = await channel._send_batch_message(
        ...
        "group:conv123",
        "sampleMarkdown",
        {"text": "hello", "title": "Minibot Reply"},
    )
...
async def test_handler_uses_voice_recognition_text_when_text_is_empty(monkeypatch) -> None:
    ...
    handler = MinibotDingTalkHandler(channel)
...
async def test_handler_processes_file_message(monkeypatch) -> None:
    ...
    async def fake_download(download_code, filename, sender_id):
        return f"/tmp/minibot_dingtalk/{sender_id}/{filename}"
    ...
    assert "/tmp/minibot_dingtalk/user1/report.xlsx" in msg.content
...
async def test_download_dingtalk_file(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "minibot.config.paths.get_media_dir",
