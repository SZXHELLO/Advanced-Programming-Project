...
from minibot.channels.email import EmailChannel
from minibot.channels.email import EmailConfig
...
def test_fetch_new_messages_parses_unseen_and_marks_seen(monkeypatch) -> None:
    ...
    fake = FakeIMAP()
    monkeypatch.setattr("minibot.channels.email.imaplib.IMAP4_SSL", lambda _h, _p: fake)
...
def test_fetch_new_messages_retries_once_when_imap_connection_goes_stale(monkeypatch) -> None:
    ...
    monkeypatch.setattr("minibot.channels.email.imaplib.IMAP4_SSL", _factory)
...
def test_fetch_new_messages_keeps_messages_collected_before_stale_retry(monkeypatch) -> None:
    ...
    monkeypatch.setattr("minibot.channels.email.imaplib.IMAP4_SSL", lambda _h, _p: FlakyIMAP())
...
def test_fetch_new_messages_skips_missing_mailbox(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "minibot.channels.email.imaplib.IMAP4_SSL",
...
async def test_send_uses_smtp_and_reply_subject(monkeypatch) -> None:
    ...
    monkeypatch.setattr("minibot.channels.email.smtplib.SMTP", _smtp_factory)
...
def test_extract_attachments_saves_pdf(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("minibot.channels.email.get_media_dir", lambda ch: tmp_path)
