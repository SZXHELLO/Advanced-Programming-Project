...
from minibot.channels.telegram import TELEGRAM_REPLY_CONTEXT_MAX_LEN, TelegramChannel, _StreamBuf
from minibot.channels.telegram import TelegramConfig
...
class _FakeBot:
    ...
    async def get_me(self):
        self.get_me_calls += 1
        return SimpleNamespace(id=999, username="minibot_test")
...
async def test_start_creates_separate_pools_with_proxy(monkeypatch) -> None:
    ...
    monkeypatch.setattr("minibot.channels.telegram.HTTPXRequest", _FakeHTTPXRequest)
    monkeypatch.setattr(
        "minibot.channels.telegram.Application",
...
async def test_send_text_retries_on_timeout() -> None:
    ...
    import minibot.channels.telegram as tg_mod
...
async def test_on_error_logs_network_issues_as_warning(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "minibot.channels.telegram.logger.warning",
        lambda message, error: recorded.append(("warning", message.format(error))),
    )
    monkeypatch.setattr(
        "minibot.channels.telegram.logger.error",
...
async def test_send_delta_stream_end_splits_oversized_reply() -> None:
    ...
    from minibot.channels.telegram import TELEGRAM_MAX_MESSAGE_LEN
...
async def test_send_remote_media_url_after_security_validation(monkeypatch) -> None:
    ...
    channel._app = _FakeApp(lambda: None)
    monkeypatch.setattr("minibot.channels.telegram.validate_url_target", lambda url: (True, ""))
...
async def test_send_blocks_unsafe_remote_media_url(monkeypatch) -> None:
    ...
    channel._app = _FakeApp(lambda: None)
    monkeypatch.setattr(
        "minibot.channels.telegram.validate_url_target",
...
async def test_group_policy_mention_accepts_text_mention_and_caches_bot_identity() -> None:
    ...
    await channel._on_message(_make_telegram_update(text="@minibot_test hi", entities=[mention]), None)
    await channel._on_message(_make_telegram_update(text="@minibot_test again", entities=[mention]), None)
...
async def test_group_policy_mention_accepts_caption_mention() -> None:
    ...
    await channel._on_message(
        _make_telegram_update(caption="@minibot_test photo", caption_entities=[mention]),
        None,
    )

    assert len(handled) == 1
    assert handled[0]["content"] == "@minibot_test photo"
...
async def test_download_message_media_returns_path_when_download_succeeds(
    ...
    monkeypatch.setattr(
        "minibot.channels.telegram.get_media_dir",
...
async def test_forward_command_preserves_dream_log_args_and_strips_bot_suffix() -> None:
    ...
    channel._handle_message = capture_handle
    update = _make_telegram_update(text="/dream-log@minibot_test deadbeef", reply_to_message=None)
...
async def test_forward_command_normalizes_telegram_safe_dream_aliases() -> None:
    ...
    channel._handle_message = capture_handle
    update = _make_telegram_update(text="/dream_restore@minibot_test deadbeef", reply_to_message=None)
