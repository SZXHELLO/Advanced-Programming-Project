...
from minibot.channels.wecom import (
    WecomChannel,
    WecomConfig,
...
async def test_download_and_save_success() -> None:
    ...
    with patch("minibot.channels.wecom.get_media_dir", return_value=Path(tempfile.gettempdir())):
...
async def test_process_image_message() -> None:
    ...
    try:
        with patch("minibot.channels.wecom.get_media_dir", return_value=Path(os.path.dirname(saved))):
            frame = _FakeFrame(body={
                "msgid": "msg_img_1",
                "chatid": "chat1",
                "from": {"userid": "user1"},
