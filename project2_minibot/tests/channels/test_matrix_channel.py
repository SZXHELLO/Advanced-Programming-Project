...
from minibot.channels.matrix import _build_matrix_text_content

import minibot.channels.matrix as matrix_module
from minibot.bus.events import OutboundMessage
from minibot.bus.queue import MessageBus
from minibot.channels.matrix import (
    MATRIX_HTML_FORMAT,
    TYPING_NOTICE_TIMEOUT_MS,
    MatrixChannel,
)
from minibot.channels.matrix import MatrixConfig
...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("minibot.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "minibot.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("minibot.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "minibot.channels.matrix.asyncio.create_task", _fake_create_task
    )
