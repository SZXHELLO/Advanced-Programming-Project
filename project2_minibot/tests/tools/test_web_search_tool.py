...
from minibot.agent.tools.web import WebSearchTool
...
async def test_brave_search(monkeypatch):
    async def mock_get(self, url, **kw):
        ...
        return _response(json={
            "web": {"results": [{"title": "MiniBot", "url": "https://example.com", "description": "AI assistant"}]}
        })

    monkeypatch.setattr(httpx.AsyncClient, "get", mock_get)
    tool = _tool(provider="brave", api_key="brave-key")
    result = await tool.execute(query="minibot", count=1)
    assert "MiniBot" in result
...
async def test_duckduckgo_search(monkeypatch):
    ...
    monkeypatch.setattr("minibot.agent.tools.web.DDGS", MockDDGS, raising=False)
    import minibot.agent.tools.web as web_mod
...
async def test_duckduckgo_timeout_returns_error(monkeypatch):
    """DDGS timeout should trigger fallback instead of crashing."""
    import threading
    ...
    result = await tool.execute(query="test")
    gate.set()
    assert "Results for: test" in result or "No results for: test" in result


@pytest.mark.asyncio
async def test_duckduckgo_ddgs_failure_falls_back_to_html(monkeypatch):
    class BrokenDDGS:
        def __init__(self, **kw):
            pass
    ...
    async def mock_get(self, url, **kw):
        assert "duckduckgo.com/html" in str(url)
        html = """
        <html><body>
          <a class="result__a" href="/l/?uddg=https%3A%2F%2Fexample.com%2Fnews">Example title</a>
...
@pytest.mark.asyncio
async def test_duckduckgo_ddgs_and_html_fallback_both_fail(monkeypatch):
    class BrokenDDGS:
        def __init__(self, **kw):
            pass
    ...
    async def mock_get(self, url, **kw):
        raise httpx.ConnectError("network down", request=httpx.Request("GET", str(url)))
