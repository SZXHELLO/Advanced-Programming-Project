...
async def test_exec_blocks_curl_metadata():
    tool = ExecTool()
    with patch("minibot.security.network.socket.getaddrinfo", _fake_resolve_private):
...
async def test_exec_blocks_wget_localhost():
    tool = ExecTool()
    with patch("minibot.security.network.socket.getaddrinfo", _fake_resolve_localhost):
...
async def test_exec_allows_curl_to_public_url():
    ...
    tool = ExecTool()
    with patch("minibot.security.network.socket.getaddrinfo", _fake_resolve_public):
...
# --- #2989: block writes to minibot internal state files -----------------
...
def test_exec_structural_validation_allows_empty_args():
    tool = ExecTool(allowed_commands={"ls": []})
    assert tool._guard_command("ls", "/tmp") is None
    assert tool._guard_command("ls -la", "/tmp") is None
    assert tool._guard_command("/usr/bin/ls -la", "/tmp") is None


def test_exec_structural_validation_blocks_unknown_command():
    tool = ExecTool(allowed_commands={"ls": []})
    result = tool._guard_command("grep -vE foo", "/tmp")
    assert result is not None
    assert "not in the allowed_commands whitelist" in result


def test_exec_structural_validation_validates_args():
    tool = ExecTool(allowed_commands={"grep": [r"^-vE\s+foo$"]})
    assert tool._guard_command("grep -vE foo", "/tmp") is None
    
    result = tool._guard_command("grep -vE bar", "/tmp")
    assert result is not None
    assert "failed validation against allowed patterns" in result


def test_exec_structural_validation_handles_pipelines():
    tool = ExecTool(allowed_commands={"ls": [], "grep": [r"^-vE\s+foo$"]})
    assert tool._guard_command("ls -la | grep -vE foo", "/tmp") is None
    
    result = tool._guard_command("ls -la | grep -vE bar", "/tmp")
    assert result is not None
    assert "failed validation against allowed patterns" in result
    
    result2 = tool._guard_command("ls -la | rm -rf /", "/tmp")
    assert result2 is not None
    assert "dangerous pattern" in result2  # Should be caught by deny_patterns first
    
    # Bypass deny pattern to check subcommand validator
    result3 = tool._guard_command("ls -la | unknowncmd", "/tmp")
    assert result3 is not None
    assert "not in the allowed_commands whitelist" in result3


def test_exec_structural_validation_handles_assignments():
    tool = ExecTool(allowed_commands={"echo": []})
    assert tool._guard_command("FOO=bar echo baz", "/tmp") is None
    
    result = tool._guard_command("FOO=bar unknowncmd baz", "/tmp")
    assert result is not None
    assert "not in the allowed_commands whitelist" in result


def test_exec_structural_validation_handles_redirects():
    tool = ExecTool(allowed_commands={"echo": [r"^hello$"]})
    assert tool._guard_command("echo hello > out.txt", "/tmp") is None
