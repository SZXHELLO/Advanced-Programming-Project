...
class TestMessageToolSuppressLogic:
    ...
    async def test_progress_hides_internal_reasoning(self, tmp_path: Path) -> None:
        ...
        # The runner may resolve a concrete tool instance via `prepare_call`,
        # so mock the actual tool's `execute` to force deterministic output.
        read_tool = loop.tools.get("read_file")
        if read_tool is not None:
            read_tool.execute = AsyncMock(return_value="ok")  # type: ignore[method-assign]
        ...
        assert progress == [
            ("Visible", False),
            ('read foo.txt', True),
            ("Observation (ok): read_file => ok", False),
        ]
