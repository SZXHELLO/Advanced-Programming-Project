...
const STORAGE_KEYS = {
    ...
    AUTH_SESSION: 'sdoc_auth_session'
};
...
const AppState = {
    ...
    save() {
        ...
        // 先保存体积较小且高频变更的数据，避免 pages 写入失败阻断 annotations 落盘
        if (this.project) {
            safeSet(STORAGE_KEYS.PROJECT_META, this.project, '项目信息');
        }
        safeSet(STORAGE_KEYS.ANNOTATIONS, this.annotations, '标注');
        safeSet(STORAGE_KEYS.CUSTOM_CHARS, this.customChars, '造字');
        safeSet(STORAGE_KEYS.PAGES, this.pages, '页面');
    },
...
const Auth = {
    API_BASE: window.SDOC_API_BASE || 'http://localhost:8000',
    ...
    isLoggedIn() {
        ...
        return Boolean(session && session.userId && session.username);
    },
    ...
    async deleteCurrentAccount() {
        ...
        const response = await fetch(`${this.API_BASE}/api/auth/account`, {
            method: 'DELETE',
            headers: {
        ...
        const payload = await response.json().catch(() => ({}));
        if (!response.ok) {
            throw new Error(payload.error || '注销失败，请稍后重试');
        }
        return payload;
    },
...
function sanitizeRedirectTarget(rawTarget) {
    if (!rawTarget || typeof rawTarget !== 'string') return 'index.html';
    if (rawTarget.includes('://') || rawTarget.startsWith('//')) return 'index.html';
    if (rawTarget.startsWith('login.html')) return 'index.html';
    return rawTarget;
}
...
function enforceAuthRouteGuard() {
    ...
    if (!loggedIn && !isPublicPage) {
        const currentPath = currentPage + window.location.search;
        Navigation.go('login.html', { redirect: currentPath });
        return false;
    }

    if (loggedIn && isPublicPage) {
        const params = Navigation.getParams();
        const redirectTarget = sanitizeRedirectTarget(params.redirect);
        Navigation.go(redirectTarget);
        return false;
    }
...
function bindLoginPageEvents() {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            hint.textContent = '登录成功，正在进入系统...';
            const params = Navigation.getParams();
            const redirectTarget = sanitizeRedirectTarget(params.redirect);
            Navigation.go(redirectTarget);
        } catch (err) {
            hint.className = 'auth-hint error';
            hint.textContent = err?.message || '登录失败，请稍后重试';
        } finally {
...
const XMLGenerator = {
    generate() {
        ...
        // sources（type: 1=图片 2=PDF；src 为相对路径）
        const sourcesType = this.getSourcesExportType(proj);
        xml += `  <sources type="${sourcesType}">\n`;
        AppState.pages.forEach((page, i) => {
            const pageNo = page.pageNo != null ? Number(page.pageNo) : i + 1;
            const src = this.getSourceExportSrc(page, i);
            xml += `    <source src="${this.escapeXml(src)}" pageno="${pageNo}"/>\n`;
        });
        xml += `  </sources>\n`;
    ...
    getSourcesExportType(proj) {
        const raw = proj.sourcesType !== undefined && proj.sourcesType !== null
            ? proj.sourcesType
            : proj.sourceType;
    ...
    isEmbeddedOrRemoteUrl(str) {
        const t = String(str || '').trim().toLowerCase();
        return t.startsWith('data:') || t.startsWith('http://') || t.startsWith('https://') || t.startsWith('blob:');
    },

    getSourceExportSrc(page, index) {
        const pageNo = page.pageNo != null ? Number(page.pageNo) : index + 1;
        const explicit = [page.sourceSrc, page.relativeSrc, page.srcPath]
            .map(v => (v != null ? String(v).trim() : ''))
            .find(s => s.length > 0);
        if (explicit && !this.isEmbeddedOrRemoteUrl(explicit)) {
            return explicit;
        }
        const img = (page.imageSrc || '').trim();
        if (img && !this.isEmbeddedOrRemoteUrl(img)) {
            return img;
        }
        return `images/page_${pageNo}.png`;
    },
    ...
    renderTextfield(ann, indentSpaces = 0, children = []) {
        ...
        if (ann.attrType === '字') {
            ...
            if ([x, y, width, height].every(Number.isFinite)) {
                ...
                positionAttr = ` x="${centerX}" y="${centerY}"`;
            }
        }
        const base = `${indent}<textfield id="${this.escapeXml(ann.id || '')}" simplified="${this.escapeXml(ann.simplifiedChar || '')}" note="${this.escapeXml(ann.note || '')}"${attrTypeAttr}${positionAttr}`;
...
function processTextResult(result) {
    ...
    const fixedColor = "#C84A32";
    const fixedAttrType = "字";
    ...
    // OCR 结果写入后立即持久化，避免跨页跳转时丢失
    AppState.save();
    if (typeof collabEmitAnnotationsAddMany === "function") {
        collabEmitAnnotationsAddMany(newAnnotations);
    }
